...
class NECAgent:
    ...
    def Q_lookahead(self, t, warmup=False):
        ...
        if warmup or len(self.transition_queue) <= t + self.lookahead_horizon:
            ...
            maxqa=self.gamma ** self.lookahead_horizon * torch.cat([dnd.lookup(state_embedding) for dnd in self.dnd_list]).max()
            return Variable(Tensor([maxqa.detach().item() + lookahead]))
